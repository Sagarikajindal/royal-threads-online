...
import { Heart, MessageCircle, Share2, Star, ChevronLeft, Truck, ShieldCheck, Sparkles } from "lucide-react";
...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          const p = data as any;
          p.product_images = (p.product_images ?? []).sort((a: any, b: any) => a.position - b.position);
          setProduct(p);
  ...
  const submitReview = async () => {
    if (!user || !product) { navigate("/auth"); return; }
    setSubmitting(true);
    const { error } = await supabase.from("reviews").upsert({
      user_id: user.id, product_id: product.id, rating: myRating, comment: myComment.trim() || null,
  ...
  if (loading) {
    return <Layout><div className="container mx-auto px-4 py-12 grid md:grid-cols-2 gap-12">
      <Skeleton className="aspect-[3/4]" /><div className="space-y-4"><Skeleton className="h-10 w-3/4" /><Skeleton className="h-6 w-1/2" /><Skeleton className="h-32" /></div></div></Layout>;
  }
  ...
  const images = product.product_images.length ? product.product_images : [{ url: "/placeholder.svg", position: 0 }];
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-6">
        <Link to="/shop" className="inline-flex items-center text-sm text-muted-foreground hover:text-primary transition-smooth">
      ...
      <section className="container mx-auto px-4 pb-12 grid lg:grid-cols-2 gap-12">
        ...
        <div>
          ...
          {images.length > 1 && (
            <div className="grid grid-cols-5 gap-2">
              {images.map((img, i) => (
                <button key={i} onClick={() => setActiveImg(i)} className={cn("aspect-square overflow-hidden border-2 transition-smooth", activeImg === i ? "border-primary" : "border-transparent")}>
        ...
        {/* INFO */}
        <div>
          <p className="text-gold-deep tracking-[0.3em] text-xs uppercase mb-3">{product.category}</p>
          <h1 className="font-serif text-4xl md:text-5xl text-primary mb-4 leading-tight">{product.name}</h1>

          {reviews.length > 0 && (
            <div className="flex items-center gap-2 mb-4">
              <div className="flex">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} size={16} className={i < Math.round(avgRating) ? "fill-gold text-gold" : "text-muted-foreground"} />
                ))}
              </div>
          ...
          <div className="flex items-baseline gap-3 mb-6">
            <span className="font-serif text-3xl text-primary">{formatINR(product.price)}</span>
            {product.original_price && product.original_price > product.price && (
              <>
                <span className="text-lg text-muted-foreground line-through">{formatINR(product.original_price)}</span>
                <Badge className="bg-primary text-primary-foreground rounded-none">{discount}% OFF</Badge>
              </>
            )}
          </div>

          {product.description && <p className="text-foreground/80 leading-relaxed mb-6">{product.description}</p>}
          ...
          <div className="flex gap-3 mb-6">
            <Button asChild size="lg" className="flex-1" disabled={!product.in_stock}>
              <a href={buildWhatsAppLink(product.name, window.location.href)} target="_blank" rel="noreferrer">
                <MessageCircle className="mr-2" size={18} /> Inquire on WhatsApp
              </a>
            </Button>
            <Button size="lg" variant="outline" onClick={toggleWishlist} aria-label="Wishlist">
              <Heart size={18} className={isWishlisted ? "fill-primary text-primary" : ""} />
            </Button>
            <Button size="lg" variant="outline" onClick={share} aria-label="Share">
              <Share2 size={18} />
            </Button>
          </div>

          <div className="grid grid-cols-3 gap-4 text-center text-xs">
            <div className="border border-border p-3"><Truck size={18} className="mx-auto text-gold-deep mb-1" /><p>Free Shipping</p></div>
            <div className="border border-border p-3"><ShieldCheck size={18} className="mx-auto text-gold-deep mb-1" /><p>Quality Assured</p></div>
            <div className="border border-border p-3"><Sparkles size={18} className="mx-auto text-gold-deep mb-1" /><p>Handcrafted</p></div>
          </div>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-12 max-w-3xl">
        <h2 className="font-serif text-3xl text-primary text-center mb-2">Customer Reviews</h2>
        ...
        {user ? (
          <div className="bg-secondary/40 p-6 mb-8">
            ...
            <div className="flex gap-1 mb-3">
              {Array.from({ length: 5 }).map((_, i) => (
                <button key={i} onClick={() => setMyRating(i + 1)} aria-label={`${i + 1} stars`}>
                  <Star size={24} className={i < myRating ? "fill-gold text-gold" : "text-muted-foreground"} />
                </button>
              ))}
            </div>
            <Textarea value={myComment} onChange={(e) => setMyComment(e.target.value.slice(0, 500))} placeholder="Tell others about the fabric, fit, craftsmanship..." rows={3} className="mb-3" />
            <Button onClick={submitReview} disabled={submitting}>{submitting ? "Posting..." : "Post Review"}</Button>
          </div>
        ) : (
        ...
        {reviews.length === 0 ? (
          <p className="text-center text-muted-foreground">Be the first to review this piece.</p>
        ) : (
            ...
            {reviews.map((r) => (
              <div key={r.id} className="border-b border-border pb-6">
                <div className="flex items-center gap-2 mb-2">
                  <div className="flex">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star key={i} size={14} className={i < r.rating ? "fill-gold text-gold" : "text-muted-foreground"} />
                    ))}
                  </div>
                  <span className="text-xs text-muted-foreground">{new Date(r.created_at).toLocaleDateString()}</span>
                </div>
                {r.comment && <p className="text-foreground/80">{r.comment}</p>}
              </div>
            ))}
          </div>
        )}
